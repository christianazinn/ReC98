...
static uint8_t keyconfig_bindings[T2_KEYCONFIG_BINDING_COUNT];
...
static bool keyconfig_file_valid(const keyconfig_file_t __ss& cfg)
{
	...
	for(i = 0; i < T2_KEYCONFIG_BINDING_COUNT; i++) {
		...
		if(cfg.bindings[i] != T2_KEYCONFIG_KEY_UNBOUND) {
			for(j = 0; j < i; j++) {
	...
	for(action = 0; action < KCA_COUNT; action++) {
		...
		for(i = 0; i < T2_KEYCONFIG_BINDINGS_PER_ACTION; i++) {
			if(cfg.bindings[(action * 2) + i] != T2_KEYCONFIG_KEY_UNBOUND) {
				bound = true;
			}
		}
		if(!bound) {
...
{
	uint8_t groups[T2_KEYCONFIG_KEY_GROUP_COUNT];
