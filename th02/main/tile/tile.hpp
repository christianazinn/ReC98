...
// Sets the [tile_ring] tile at (left, y) to the given [image], then blits it
// at (⌊left/8⌋*8, ⌊y/TILE_H⌋*TILE_H).
// (This function would have been easier to describe if it only used [left] to
// locate the tile and then blit it at its standard position, but it can fact
// also blit a tile *between* two horizontal ones…)
...
// If `true`, the next call to tiles_egc_render() will unconditionally redraw
// all tiles and then reset this flag to `false`.
extern bool tiles_egc_render_all;

// Resets a random bunch of data to its initial values at the beginning of a
// stage.
...
// scrolled-in tile lines from the tile source area to the playfield. Can only
// blit lines from a single tile row, so the caller must ensure that the passed
// [speed] values always end up exactly on a [TILE_H] boundary.
...
// Fills [tile_ring] with the initial screen of tiles at the bottom/start of
// the map, and blits them to the playfield area in VRAM. Reads tile images
// from our data segment, the tile source area is neither read from nor written
// to.
void tiles_fill_and_put_initial(void);

// Marks all tiles for redrawing that lie within the given area of the screen.
...
// [tile_mode]. For TM_TILES, the EGC is assumed to be active and initialized
// for a copy.
