...
extern "C" void pascal near verdict_bitmap_snap(size_t bitmap_offset)
{
	...
	// end, so the source is DS:SI and the destination ES:DI = _DATA:[bitmap].
	// `rep movsw` leaves both advanced by one row's worth, and only SI needs
	// the extra step to the next VRAM row -- [verdict_bitmap] is contiguous.
	asm {
		push	ds;
		push	ds;
		pop 	es;
