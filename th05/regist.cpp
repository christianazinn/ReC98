...
/// ------------------------------------------
/// Both pragmas must precede every emitted byte (kb/codegen/0138). The merged
/// product hosts the former hi_end.cpp registration contribution and this
/// file's glyph-ball contribution in this single translation unit:
///
/// • -zC keeps all of those routines in SCORE_TEXT.
/// • -zP is required because glyphball_spawn() and
///   glyphballs_update_and_render() both
...
bool pascal near hiscore_scoredat_load_for(int pc)
{
	return scoredat_load(
...
/// Defined by th04/hiscore/regist_view.cpp above. `near` matches the original
/// SCORE_TEXT contribution.
void pascal near name_put(int place, playchar_t pc, unsigned char cursor);
/// -----------------------------

/// Coordinates
/// -----------
/// The merged product includes th04/hiscore/regist_view.cpp above, so these
/// routines reuse that fragment's constants. Every literal in this block
/// falls out of them:
...
// The screen that drives all of the above. ONE body for both games; the
// merged TH04 product includes the same file from th04/regist.cpp.
