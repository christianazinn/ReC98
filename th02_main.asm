...
; Phases 1 and 2's per-frame patterns, stones_119CD() and stones_11A87(),
; and the per-rank parameter seeder stones_11997() are
; th02/main/boss/b3.cpp, together with every one of the fight's eleven
; bullet patterns, the take-over animation, the background-eating
; animation, stones_121BA(), stones_update() and the three functions
; below this block. `angle_1EB28`, `word_22FAC` and `byte_22FAE` (now
; `stones_laser_left` / `stones_laser_return_delay`), `angle_22FAF`,
; `angle_22FB0` and the five-byte `angle_22FB1` carry kb/codegen/0123
; aliases for their sake. `boss_rank_param` needed nothing: the dump's
; `byte_2066E`, `byte_2066F`, `group_20670` and `byte_20671` are cells 0
; to 3 of an extent that has been published all along.
;
; The two stones_* procs still above dispatch from stones_update() and
; are published for that and for nothing else. One of them,
; stones_11766(), is `pascal`, so its alias is UPPER case and carries no
; leading underscore (kb/codegen/0102 + 0103).
