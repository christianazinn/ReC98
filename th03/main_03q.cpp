...
extern "C" void pascal near yumemi_10669(void)
{
	register int i;

	sub_F356();
	if((word_1F3B0 % static_cast<uint16_t>(byte_1F3A4)) == 0) {
		bullet_template.type = BT_BULLET16_DEFAULT;
		if(word_1F3B0 < 0x40) {
			bullet_template.center.x.v = word_20E50;
			bullet_template.center.y.v = word_20E52;
			bullet_template.angle = -0x40;
			bullet_template.speed.v = (6 << 4);
		...
		if(word_1F3B0 >= 0x20) {
			...
			bullet_template.speed.v = ((2 << 4) + 4);
			bullet_template.center.y.v = 0;
			for(i = 0; i < 12; i++) {
				bullet_template.center.x.v = randring_far_next16_mod(
					PLAYFIELD_W << 4
				);
...
extern "C" void pascal far gba_boss_update_yumemi(void)
{
	...
	if(sub_F402()) {
		...
		byte_1F3A0 = ((gba_boss_level / 2) + 0x10);
		byte_1F3A1 = (gba_boss_level + 0x40);
		byte_1F3A2 = (0x12 - gba_boss_level);
		byte_1F3A3 = ((gba_boss_level * 4) + 0x50);
		byte_1F3A4 = (0x10 - (gba_boss_level / 2));
	}
	...
	sub_F512();
	word_1F3B0++;
	word_20E50 = (word_1F33E + TO_SP(9));
	word_20E52 = (word_1F340 + TO_SP(-24));

	switch(byte_1F34F) {
	case 0:
		if((word_1F3B0 == 0x18) || (word_1F3B0 == 0x48)) {
			snd_se_reset();
			snd_se_play(5);
		}
		if(word_1F3B0 != 0x60) {
