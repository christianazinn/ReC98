...
#include "th03/replay_options.hpp"
...
void near cfg_load(void)
{
	...
	replay_recording_enabled_set(
		(cfg.opts.language & T3_CFG_REPLAY_RECORDING_DISABLED) == 0
	);
...
inline void cfg_save_bytes(cfg_t &cfg, size_t bytes) {
	...
	cfg.opts.language = static_cast<uint8_t>(
		language_resident() |
		(replay_recording_enabled() ? 0 : T3_CFG_REPLAY_RECORDING_DISABLED) |
...
static bool near vs_start(bool select_characters)
{
	...
	if((sel == VS_CPU_CPU) || !replay_recording_enabled()) {
		replay_resident_handoff_clear();
	} else {
		replay_resident_handoff_set(T3_REPLAY_RES_MODE_USER_RECORD);
		replay_resident_handoff_slot_set(T3_REPLAY_USER_SLOT_NONE);
...
static void replay_menu_render(uint8_t sel, uint8_t top)
{
	...
	{
		// Assemble this short label directly so its literals don't move the
		// original OP data that follows the replay browser additions.
		replay_menu_line[0] = 'R';
		replay_menu_line[1] = 'e';
		replay_menu_line[2] = 'p';
		replay_menu_line[3] = 'l';
		replay_menu_line[4] = 'a';
		replay_menu_line[5] = 'y';
		replay_menu_line[6] = 's';
		replay_menu_line[7] = ':';
		replay_menu_line[8] = ' ';
		replay_menu_line[9] = 'O';
		if(replay_recording_enabled()) {
			replay_menu_line[10] = 'n';
			replay_menu_line[11] = '\0';
		} else {
			replay_menu_line[10] = 'f';
			replay_menu_line[11] = 'f';
			replay_menu_line[12] = '\0';
		}
		if(menu_font) {
			menu_font_put(
...
bool near replay_menu(void)
{
	...
	while(1) {
		...
		if(!detail_view && replay_toggle && !replay_toggle_prev) {
			replay_recording_enabled_set(!replay_recording_enabled());
			cfg_save();
			replay_menu_render(sel, top);
		}
...
static void near title_credit_put(void)
{
	...
	TITLE_CREDIT_QUAD(4, 0x39312E34UL); // "4.19"
