...
#if (GAME == 5)
// Waits the given amount of frames (0 = forever) for the OK, Shot, or Cancel
// button to be pressed, while showing the ⏎ Return key animation in a blocking
// way.
...
bool16 pascal near cutscene_script_load(const char* fn)
{
	...
	#if (GAME >= 4)
		#define cutscene_file_ropen language_asset_file_ropen
		#define cutscene_file_close language_asset_file_close
	#else
		#define cutscene_file_ropen file_ropen
		#define cutscene_file_close file_close
	#endif
	if(!cutscene_file_ropen(fn)) {
		return true;
	}
	...
	cutscene_file_close();
	#undef cutscene_file_close
	#undef cutscene_file_ropen
	return false;
}
