...
static bool replay_stock_option_initialized = false;
static bool replay_stock_option_input_allowed;
...
{
	if(func == ROBF_PLAYCHAR_MENU) {
		return playchar_menu();
	}
	if(func == ROBF_EXIT_PREPARE) {
		main_cdg_free();
		cfg_save();
	} else if(func == ROBF_REGIST_VIEW_MENU) {
		regist_view_menu();
	} else if(func == ROBF_MUSICROOM_MENU) {
		musicroom_menu();
	} else if(func == ROBF_MAIN_CDG_LOAD) {
		main_cdg_load();
	}
	return false;
}
...
// trampoline above.
#if (GAME == 4)
	// 11 * 32 + 12 = 364 bytes. Together with replay_op_bridge(),
	// this retains the stock replacement span and following public offsets.
...
#else
	// 13 * 32 + 9 = 425 bytes. Together with replay_op_bridge(),
	// this retains the stock replacement span and following public offsets.
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
...
void near option_update_and_render(void)
{
	language_option_update_and_render();
}

// Keep the original [option_update_and_render()] span intact. The complete
// updater lives in REPLAY_OP_TEXT, so this is unreachable padding rather than
// a behavioral change to any protected stock entry point.
#if (GAME == 4)
	// 24 * 32 + 27 = 795 bytes.
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
	...
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
#else
	// 27 * 32 + 16 = 880 bytes.
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
	#pragma codestring "\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90\x90"
