...
/// the object that already sits immediately before it: th04/hi_end.cpp for
/// TH04 (this file is the last `#include` of th04/hiscore/end.cpp) and
...
#if (GAME == 5)
/// [measured] These four all live in th05/hi_end.cpp's SCORE_TEXT
/// contribution — a different object, but the same segment and the same
/// group, so `near` is correct. Spelled exactly as their defining translation
/// unit spells them; TH04 gets them from that unit directly, because there
/// this file is part of the same `#include` chain.
...
/// [measured] Every string this screen passes is a `_DATA` byte of the root
/// dump, not a literal of this translation unit, and it has to stay that way:
/// the build compiles with `-d`, which merges duplicate string literals, and
/// the two Japanese messages below are *byte-identical to each other*. Two
/// separate storage copies is what the original has, so emitting them from
/// C++ would both collapse them into one and move them out of the dump's
/// `_DATA` contribution, shifting every following byte of it. Declared here
/// and published there instead — th03/hiscore/regist.cpp does exactly this
/// for th03/mainl/rsedat[data].asm's filenames, and the `_SCOREDAT_FN*`
/// group three lines above these in both dumps is already the same pattern.
...
// ZUN bloat: The same string twice, once as its own drop shadow.
extern const shiftjis_t SLOW_MODE_MSG_SHADOW[];
...
void near regist_menu(void)
{
	...
	// TH04 resolves this to th02/formats/pi.h's inlining macro, TH05 to
	// th05/formats/pi.cpp's actual function; the include block above is what
	// picks between them.
	pi_free(0);
	...
	if(resident->turbo_mode || (rank == RANK_EXTRA)) {
		...
		entered_place = -1;
		places_put(playchar_arg(playchar));
		graph_putsa_fx(124, 196, 9, SLOW_MODE_MSG_SHADOW);
