...
void sariel_entrance(int8_t)
{
	...
	for(pixel_t line_on_top = (RES_Y - 1); line_on_top >= 0; line_on_top--) {
		...
		egc.rect_interpage(0, line_on_top, RES_X, 1, 1);
		frame_delay(1);
	}
...
void sariel_setup(void)
{
	ent_shield.pos_set(SHIELD_LEFT, SHIELD_TOP, 0, RES_X, 0, RES_Y);
	...
	for(int i = 0; i < BIRD_COUNT; i++) {
		...
		case BPG_RANDOM_RAIN:
			pellets_add_single_rain(
				pellet_left, pellet_top, ((irand() & 0x7F) + 0x80), 4.0f
			);
			break;
		}
	}
