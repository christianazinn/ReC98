...
int CBossEntity::load_inner(const char fn[PF_FN_LEN], int slot)
{
	...
	if(!bos_header_only) {
		...
		for(int i = 0; bos_image_count > i; i++) {
			...
			bos_file_get(reinterpret_cast<uint8_t *>(img.alpha), plane_size);
			bos_file_get(reinterpret_cast<uint8_t *>(img.planes.B), plane_size);
			bos_file_get(reinterpret_cast<uint8_t *>(img.planes.R), plane_size);
			bos_file_get(reinterpret_cast<uint8_t *>(img.planes.G), plane_size);
			bos_file_get(reinterpret_cast<uint8_t *>(img.planes.E), plane_size);
			#undef img
		}
	}
