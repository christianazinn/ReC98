...
static const screen_x_t T1REPLAY_OP_BROWSER_SLOT_LEFT = 64;
static const screen_x_t T1REPLAY_OP_BROWSER_MARKER_LEFT = 48;
static const screen_x_t T1REPLAY_OP_BROWSER_NAME_LEFT = 120;
static const screen_x_t T1REPLAY_OP_BROWSER_SCORE_LEFT = 392;
static const screen_x_t T1REPLAY_OP_BROWSER_STAGE_LEFT = 540;
static const screen_x_t T1REPLAY_OP_DETAIL_NAME_LEFT = 168;
static const screen_x_t T1REPLAY_OP_DETAIL_VALUE_RIGHT = 304;
static const screen_x_t T1REPLAY_OP_DETAIL_SPLIT_STAGE_LEFT = 352;
static const screen_x_t T1REPLAY_OP_DETAIL_SPLIT_SCORE_RIGHT = 608;
static const screen_y_t T1REPLAY_OP_DETAIL_TOP = 80;
static const screen_y_t T1REPLAY_OP_DETAIL_ROW_H = 24;
static const uint8_t T1REPLAY_OP_SAVED_WAIT = 0xFF;
static const screen_y_t T1REPLAY_OP_LINE_H = GLYPH_H;
static const screen_y_t T1REPLAY_OP_BROWSER_LINE_H = 24;
static const screen_y_t T1REPLAY_OP_BROWSER_LINE_TOP = 112;
...
static const vc_t T1REPLAY_OP_COL_HEADER = 9;
...
static const screen_x_t T1REPLAY_OP_SAVE_DIFF_CENTER = 365;
...
static bool t1replay_op_prev_replay_toggle;
...
static bool t1replay_op_practice_seed_is_random(void);
static void t1replay_op_practice_seed_random_set(bool random);
...
static bool t1replay_op_restart_state_normal_arm(void)
{
	return t1replay_op_restart_state_store(T1RRK_NORMAL, 0);
}

static bool t1replay_op_restart_state_practice_arm(
	const t1replay_practice_start_t *start
)
{
...
static unsigned t1replay_op_header_wire_size(const t1replay_header_t *header)
{
	...
	if(
		...
		(header->version == T1REPLAY_VERSION_TELEMETRY) &&
		(header->header_size == T1REPLAY_HEADER_SIZE_TELEMETRY)
	) {
		return T1REPLAY_HEADER_SIZE_TELEMETRY;
	}
	if(
		t1replay_op_magic_matches(header->magic, '6') &&
		(header->version == T1REPLAY_VERSION_PREVIOUS) &&
	...
	if(
		t1replay_op_magic_matches(header->magic, '8') &&
		(header->version == T1REPLAY_VERSION) &&
		(header->header_size == T1REPLAY_HEADER_WIRE_SIZE)
	) {
...
static unsigned t1replay_op_header_checksum_size(
	const t1replay_header_t *header
)
{
	return ((header->version == T1REPLAY_VERSION)
		? T1REPLAY_HEADER_SIZE : t1replay_op_header_wire_size(header));
}

static bool t1replay_op_header_has_accelerator(
	const t1replay_header_t *header
)
{
	return (
...
static bool t1replay_op_dos_datetime_valid(
	const t1replay_header_t *header, unsigned header_size
)
{
	uint16_t date = header->dos_date;
	uint16_t time = header->dos_time;
...
static bool t1replay_op_header_valid(t1replay_header_t *header)
{
	...
	if(
		...
		!t1replay_op_dos_datetime_valid(header, header_size)
	) {
		return false;
	}
	...
	header_checksum = t1replay_op_fnv1a(
		...
		t1replay_op_header_checksum_size(header)
	);
...
{
	unsigned header_size = t1replay_op_header_checksum_size(header);
...
)
{
	...
	if(
		...
		((header_size = t1replay_op_header_wire_size(&result.header)) != 0) &&
		...
		((prefix_size == T1REPLAY_HEADER_SIZE_LEGACY) ||
		 (fread(
			(reinterpret_cast<uint8_t *>(&result.header) +
			 T1REPLAY_HEADER_SIZE_LEGACY),
			1, (prefix_size - T1REPLAY_HEADER_SIZE_LEGACY), fp
		  ) == (prefix_size - T1REPLAY_HEADER_SIZE_LEGACY))
		)
	) {
...
static bool t1replay_op_pending_name_commit(const uint8_t *name)
{
	...
	valid = (
		(fseek(fp, 0L, SEEK_SET) == 0) &&
		(fwrite(
			&pending.header, 1, sizeof(pending.header), fp
...
bool t1replay_op_record_prepare(void)
{
	...
	if(
		!t1replay_op_restart_practice_armed &&
...
)
{
	...
	if(!t1replay_op_practice_enter(rank, lives, bombs, rand)) {
		return false;
	}
	...
	if(!t1replay_op_restart_state_practice_arm(&t1replay_practice_start)) {
		t1replay_process_milestone(T1RPM_RECORD_COMMAND_WRITE_FAILED);
		return false;
	}
	t1replay_process_milestone(T1RPM_PRACTICE_RESTART_ARMED);
	return true;
}
#endif
...
static bool t1replay_op_panel_load(const char *fn, uint8_t panel_kind)
{
	...
	if(!buffer) {
		#if T1REPLAY_PROCESS_MILESTONES
			t1replay_process_milestone(T1RPM_PRACTICE_PANEL_ALLOC_FAILED);
		#endif
		return false;
	}
	#if T1REPLAY_PROCESS_MILESTONES
...
static bool t1replay_op_panel_restore(bool name_entry)
{
	char fn[9];

	t1replay_op_background_fn(fn, name_entry);
	return t1replay_op_panel_show(
		fn, (name_entry ? T1OPK_NAME_ENTRY : T1OPK_REPLAY)
	);
}

static bool t1replay_op_practice_panel_restore(void)
{
	char fn[11];
...
)
{
	...
	case T1ROW_DATE: T1ROW_JP4(0x93, 0xFA, 0x95, 0x74); break;
...
static char *t1replay_op_word_append(char *p, t1replay_op_word_t word)
{
	...
	case T1ROW_DATE: T1ROW_WORD4('D', 'A', 'T', 'E'); break;
	case T1ROW_SPLITS:
	...
	case T1ROW_ADVANCED_SETTINGS:
		T1ROW_WORD4('A', 'd', 'v', 'a'); T1ROW_WORD4('n', 'c', 'e', 'd');
		T1ROW_SPACE(); T1ROW_WORD4('S', 'e', 't', 't'); T1ROW_WORD4('i', 'n', 'g', 's'); break;
	...
	case T1ROW_RANDOM: T1ROW_WORD4('R', 'a', 'n', 'd'); T1ROW_WORD2('o', 'm'); break;
...
static char *t1replay_op_date_append(
...
)
{
	...
	if(t1replay_op_header_wire_size(header) < T1REPLAY_HEADER_SIZE) {
		*p++ = '-';
		return p;
	}
	year = static_cast<uint16_t>(1980 + (date >> 9));
	month = static_cast<uint8_t>((date >> 5) & 0x0F);
	day = static_cast<uint8_t>(date & 0x1F);
	p = t1replay_op_uint_append(p, month, 2);
	*p++ = '-';
	p = t1replay_op_uint_append(p, day, 2);
	*p++ = '-';
	return t1replay_op_uint_append(p, year, 4);
}
...
static void t1replay_op_text_center_at(
	screen_x_t center, screen_y_t y, vc_t col, char *end
)
{
	*end = '\0';
	if(replay_op_font) {
		replay_op_font_put_centered(center, y, t1replay_op_text, col);
	} else {
		graph_putsa_fx(
			static_cast<screen_x_t>(
				center -
				(text_extent_fx((col | T1REPLAY_OP_FX), t1replay_op_text) / 2)
			),
			y, (col | T1REPLAY_OP_FX), t1replay_op_text
		);
	}
...
static void t1replay_op_text_right(
	screen_x_t right, screen_y_t y, vc_t col, char *end
)
{
	*end = '\0';
	if(replay_op_font) {
		replay_op_font_put_right(right, y, t1replay_op_text, col);
	} else {
		graph_putsa_fx(
			static_cast<screen_x_t>(
				right - text_extent_fx((col | T1REPLAY_OP_FX), t1replay_op_text)
			),
			y, (col | T1REPLAY_OP_FX), t1replay_op_text
		);
	}
...
static void t1replay_op_text_numeric_cells(
	screen_x_t left, screen_y_t y, vc_t col, char *end
)
{
	*end = '\0';
	if(replay_op_font) {
		replay_op_font_put_numeric_cells(
			left, y, t1replay_op_text,
			static_cast<unsigned>(end - t1replay_op_text), col
		);
	} else {
		graph_putsa_fx(left, y, (col | T1REPLAY_OP_FX), t1replay_op_text);
	}
...
static void t1replay_op_text_numeric_cells_right(
	screen_x_t right, screen_y_t y, vc_t col, char *end
)
{
	unsigned cells = static_cast<unsigned>(end - t1replay_op_text);
	...
	*end = '\0';
	if(replay_op_font) {
		replay_op_font_put_numeric_cells(
			...
			y, t1replay_op_text, cells, col
		);
	} else {
		t1replay_op_text_right(right, y, col, end);
	}
}

static void t1replay_op_text_slot_cells(
	screen_x_t left, screen_y_t y, vc_t col, char *end
)
{
...
static void t1replay_op_input_read(t1replay_op_input_t& input)
{
	...
	if(t1replay_op_wait_release) {
		input.up = input.down = input.left = input.right = input.ok = input.enter = input.strike = input.cancel = input.replay_toggle = false;
		...
		input.replay_toggle = (
			replay_toggle && !t1replay_op_prev_replay_toggle
		);
	...
	t1replay_op_prev_replay_toggle = replay_toggle;
}
...
static void t1replay_op_input_reset(void)
{
	...
	t1replay_op_prev_replay_toggle = false;
...
static void t1replay_op_save_metadata_render(
	const t1replay_header_t *header
)
{
	char *p;
	...
	p = t1replay_op_uint_append(
		t1replay_op_text, header->summary.final_score, 1
	);
	t1replay_op_text_numeric_cells_right(
		T1REPLAY_OP_SAVE_POINT_RIGHT, T1REPLAY_OP_SAVE_NAME_TOP,
		T1REPLAY_OP_COL_VALUE, p
	);
	p = t1replay_op_date_append(t1replay_op_text, header);
	t1replay_op_text_center_at(
		T1REPLAY_OP_SAVE_DATE_CENTER, T1REPLAY_OP_SAVE_METADATA_TOP,
		T1REPLAY_OP_COL_VALUE, p
	);
	p = t1replay_op_text;
	switch(header->start.rank) {
	case RANK_EASY: *p++ = 'E'; break;
	case RANK_NORMAL: *p++ = 'N'; break;
	case RANK_HARD: *p++ = 'H'; break;
	case RANK_LUNATIC: *p++ = 'L'; break;
	default: *p++ = '-'; break;
	}
	t1replay_op_text_center_at(
		T1REPLAY_OP_SAVE_DIFF_CENTER, T1REPLAY_OP_SAVE_METADATA_TOP,
		T1REPLAY_OP_COL_VALUE, p
	);
	p = t1replay_op_route_append(t1replay_op_text, header->start.route);
	t1replay_op_text_center_at(
		T1REPLAY_OP_SAVE_ROUTE_CENTER, T1REPLAY_OP_SAVE_METADATA_TOP,
		T1REPLAY_OP_COL_VALUE, p
	);
	p = t1replay_op_uint_append(
		t1replay_op_text, header->summary.final_stage_id + 1, 1
	);
	t1replay_op_text_center_at(
		T1REPLAY_OP_SAVE_STAGE_CENTER, T1REPLAY_OP_SAVE_METADATA_TOP,
		T1REPLAY_OP_COL_VALUE, p
	);
}
...
static void t1replay_op_name_render(void)
{
	...
	if(!t1replay_op_panel_restore(true)) {
		return;
	}
	t1replay_op_name_line_render();
	t1replay_op_pending_read(pending);
	if(pending.valid) {
		t1replay_op_save_metadata_render(&pending.header);
	}
...
static char *t1replay_op_save_word_append(
	char *p, t1replay_op_word_t word
)
{
	if(t1_language_get() == T1LANG_JAPANESE) {
		...
		if(t1replay_op_word_japanese_append(localized, word)) {
			return localized;
		}
	}
	return t1replay_op_word_append(p, word);
...
static void t1replay_op_save_text_center(
	screen_y_t y, vc_t col, char *end
)
{
	if(t1_language_get() != T1LANG_JAPANESE) {
		t1replay_op_text_center(y, col, end);
		return;
	}
	*end = '\0';
	graph_putsa_fx(
		static_cast<screen_x_t>(
			(RES_X - text_extent_fx(
				(col | T1REPLAY_OP_FX), t1replay_op_text
			)) / 2
		),
		y, (col | T1REPLAY_OP_FX), t1replay_op_text
	);
...
static void t1replay_op_save_decision_render(void)
{
	...
	p = t1replay_op_save_word_append(
		t1replay_op_text,
		t1replay_op_save_pending ? T1ROW_OVERWRITE : T1ROW_SAVE_REPLAY
	);
	t1replay_op_save_text_center(y, T1REPLAY_OP_COL_HEADER, p);
	y += (T1REPLAY_OP_LINE_H * 2);
	col = ((t1replay_op_sel == 0) ?
		T1REPLAY_OP_COL_SELECTED : T1REPLAY_OP_COL_LABEL);
	p = t1replay_op_save_word_append(t1replay_op_text, T1ROW_SAVE);
	t1replay_op_save_text_center(y, col, p);
	y += T1REPLAY_OP_LINE_H;
	col = ((t1replay_op_sel == 1) ?
		T1REPLAY_OP_COL_SELECTED : T1REPLAY_OP_COL_LABEL);
	p = t1replay_op_save_word_append(t1replay_op_text, T1ROW_DISCARD);
	t1replay_op_save_text_center(y, col, p);
}
...
static bool t1replay_op_detail_settings_focused(void)
{
...
)
{
	...
	if(final_header.version == T1REPLAY_VERSION_PREVIOUS) {
	...
	if(!source || !candidate ||
		(fwrite(&final_header, 1, sizeof(final_header), candidate) !=
		 sizeof(final_header)) ||
		(fseek(source, static_cast<long>(sizeof(final_header)), SEEK_SET) != 0)) {
		goto done;
	}
	tail_left = (pending_header->input_offset - sizeof(final_header));
	while(tail_left != 0) {
		want = static_cast<unsigned>(
			(tail_left > sizeof(bytes)) ? sizeof(bytes) : tail_left
		);
		if((fread(bytes, 1, want, source) != want) ||
...
static void t1replay_op_detail_render(void)
{
	...
	*p++ = ' ';
	p = t1replay_op_uint_append(p, slot_id, 1);
	...
	#define T1REPLAY_OP_DETAIL_LINE(top, label_word, value_append) \
		...
		t1replay_op_text_left(top, T1REPLAY_OP_COL_LABEL, p); \
		p = t1replay_op_text; \
		p = (value_append); \
		t1replay_op_text_right( \
			T1REPLAY_OP_DETAIL_VALUE_RIGHT, top, T1REPLAY_OP_COL_VALUE, p \
		)
	...
	*p++ = ' ';
	...
	if(slot.header.flags & T1REPLAY_FLAG_PRACTICE) {
		...
		p = t1replay_op_word_append(t1replay_op_text, T1ROW_ADVANCED_SETTINGS);
		t1replay_op_text_left(
			280,
			(t1replay_op_detail_settings_focused() ?
				T1REPLAY_OP_COL_SELECTED : T1REPLAY_OP_COL_HEADER),
			p
		);
	} else {
		T1REPLAY_OP_DETAIL_LINE(
	...
	split_page = static_cast<uint8_t>(
	...
	*p++ = '/';
	...
	t1replay_op_text_put(
		T1REPLAY_OP_DETAIL_SPLIT_STAGE_LEFT, y, T1REPLAY_OP_COL_HEADER, p
	);
	y = 112;
...
static void t1replay_op_detail_settings_modal(const t1replay_header_t& header)
{
	char *p;
	screen_y_t y = 96;
	...
	z_grcg_boxfill(72, 72, 568, 336, V_BLACK);
	p = t1replay_op_word_append(t1replay_op_text, T1ROW_ADVANCED_SETTINGS);
	t1replay_op_text_center_at(320, y, T1REPLAY_OP_COL_HEADER, p);
	y += 40;

	#define T1REPLAY_OP_SETTINGS_LINE(label_word, value_append) \
		p = t1replay_op_word_append(t1replay_op_text, label_word); \
		t1replay_op_text_left(y, T1REPLAY_OP_COL_LABEL, p); \
		p = t1replay_op_text; \
		p = (value_append); \
		t1replay_op_text_right( \
			T1REPLAY_OP_DETAIL_VALUE_RIGHT, y, T1REPLAY_OP_COL_VALUE, p \
		); \
		y += 24
	T1REPLAY_OP_SETTINGS_LINE(
		T1ROW_START_SCORE,
		t1replay_op_uint_append(p, header.start.score, 1)
	);
	T1REPLAY_OP_SETTINGS_LINE(
		T1ROW_START_LIVES,
		t1replay_op_uint_append(p, header.start.rem_lives, 1)
	);
	T1REPLAY_OP_SETTINGS_LINE(
		T1ROW_START_BOMBS,
		t1replay_op_uint_append(p, header.start.rem_bombs, 1)
	);
	T1REPLAY_OP_SETTINGS_LINE(
		T1ROW_START_POINT_VALUE,
		t1replay_op_uint_append(p, header.start.point_value, 1)
	);
	T1REPLAY_OP_SETTINGS_LINE(
		T1ROW_START_PELLET_SPEED,
		t1replay_op_pellet_speed_append(p, header.start.pellet_speed)
	);
	T1REPLAY_OP_SETTINGS_LINE(
		T1ROW_RNG_SEED,
		t1replay_op_uint_append(p, header.start.resident_rand, 1)
	);
	#undef T1REPLAY_OP_SETTINGS_LINE
...
static bool t1replay_op_replay_render(void)
{
	t1replay_op_slot_t slot;
	char *p;
	screen_y_t y = 80;
	...
	p = t1replay_op_word_append(t1replay_op_text, T1ROW_SLOT);
	t1replay_op_text_put(T1REPLAY_OP_BROWSER_SLOT_LEFT, y, T1REPLAY_OP_COL_HEADER, p);
	p = t1replay_op_word_append(t1replay_op_text, T1ROW_NAME);
	t1replay_op_text_put(T1REPLAY_OP_BROWSER_NAME_LEFT, y, T1REPLAY_OP_COL_HEADER, p);
	p = t1replay_op_word_append(t1replay_op_text, T1ROW_SCORE);
	t1replay_op_text_put(T1REPLAY_OP_BROWSER_SCORE_LEFT, y, T1REPLAY_OP_COL_HEADER, p);
	p = t1replay_op_word_append(t1replay_op_text, T1ROW_STAGE);
	t1replay_op_text_put(T1REPLAY_OP_BROWSER_STAGE_LEFT, y, T1REPLAY_OP_COL_HEADER, p);
	y = T1REPLAY_OP_BROWSER_LINE_TOP;
	...
	*p++ = '/'; *p++ = '1'; *p++ = '0';
	t1replay_op_text_center(356, T1REPLAY_OP_COL_LABEL, p);
	p = t1replay_op_text;
	memcpy(p, "Replays: ", 9); p += 9;
	if(t1_replay_recording_enabled()) {
		*p++ = 'O'; *p++ = 'n';
	} else {
		*p++ = 'O'; *p++ = 'f'; *p++ = 'f';
...
static bool t1replay_op_practice_render(void)
{
	char *p;
	screen_y_t y = 16;
	uint8_t row = 0;
	...
	if(!t1replay_op_practice_panel_restore()) {
		return false;
	}
	p = t1replay_op_word_append(t1replay_op_text, T1ROW_PRACTICE);
	*p++ = ' ';
	...
	#define T1REPLAY_OP_PRACTICE_LINE(label_word, value_append) \
		...
		value_col = label_col; \
		p = t1replay_op_word_append(t1replay_op_text, label_word); \
		t1replay_op_text_put(104, y, label_col, p); \
		p = (value_append); \
		t1replay_op_text_right(512, y, value_col, p); \
		y += 22; row++
	...
	T1REPLAY_OP_PRACTICE_LINE(
		T1ROW_RNG_SEED,
		(t1replay_op_practice_seed_is_random()
			? t1replay_op_word_append(t1replay_op_text, T1ROW_RANDOM)
			: t1replay_op_uint_append(
				t1replay_op_text, t1replay_practice_start.rand, 1
			)
		)
	);
...
// Practice has no pages. Reuse the browser page byte for its transient seed
// mode so the additive OP tail keeps its pinned BSS extent.
static bool t1replay_op_practice_seed_is_random(void)
{
	return (t1replay_op_page != 0);
}

static void t1replay_op_practice_seed_random_set(bool random)
{
	t1replay_op_page = (random ? 1 : 0);
}
...
bool t1replay_op_practice_enter(int8_t rank, int8_t lives, int8_t bombs, uint32_t rand)
{
	...
	#if T1REPLAY_PROCESS_MILESTONES
		t1replay_process_milestone(T1RPM_PRACTICE_FONT_READY);
	#endif
...
t1replay_op_result_t t1replay_op_replay_update(void)
{
	...
	if(
		!t1replay_op_save_pending && !t1replay_op_name_active &&
		!t1replay_op_save_decision && !t1replay_op_detail_active() &&
		input.replay_toggle
	) {
		if(t1replay_op_settings_write(
			t1_language_get(), !t1_replay_recording_enabled()
		)) {
			t1replay_op_replay_render();
		}
		return result;
	...
	if(t1replay_op_detail_active()) {
		...
		if(input.left) {
			...
			t1replay_op_slot_read(result.slot, slot);
			if(
				(slot.header.flags & T1REPLAY_FLAG_PRACTICE) &&
				((t1replay_op_horizontal_hold & T1REPLAY_OP_DETAIL_PAGE_MASK) == 1)
			) {
				t1replay_op_horizontal_hold |= T1REPLAY_OP_DETAIL_SETTINGS_FOCUS;
			}
		...
		if(input.right) {
			if(t1replay_op_detail_settings_focused()) {
				t1replay_op_horizontal_hold &= T1REPLAY_OP_DETAIL_PAGE_MASK;
				t1replay_op_detail_render();
				return result;
			}
...
static void t1replay_op_practice_change(int delta, bool fast)
{
	...
	case T1OPR_RNG_SEED:
		if(t1replay_op_practice_seed_is_random()) {
			t1replay_op_practice_seed_random_set(false);
			t1replay_practice_start.rand = ((delta > 0) ? 0 : 0xFFFFFFFFUL);
		} else if(delta > 0) {
			if(t1replay_practice_start.rand == 0xFFFFFFFFUL) {
				t1replay_op_practice_seed_random_set(true);
			} else {
				uint32_t amount = (fast ? 256UL : 1UL);
				t1replay_practice_start.rand = (
					(t1replay_practice_start.rand > (0xFFFFFFFFUL - amount))
						? 0xFFFFFFFFUL
						: (t1replay_practice_start.rand + amount)
				);
			}
		} else if(t1replay_practice_start.rand == 0) {
			t1replay_op_practice_seed_random_set(true);
		} else {
			uint32_t amount = (fast ? 256UL : 1UL);
			t1replay_practice_start.rand = (
...
t1replay_op_result_t t1replay_op_practice_update(void)
{
	...
	if(input.enter && t1replay_op_practice_field_is_numeric(t1replay_op_sel)) {
		...
		if(t1replay_op_sel == T1OPR_START) {
			...
			if(t1replay_op_restart_state_practice_arm(&restart_start)) {
